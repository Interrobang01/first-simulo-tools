    ...
    if selected_object_guid ~= nil then
        ...
        runtime_eval{
            ...
            code = [[
                ...
                local dupe = Input:key_pressed("AltLeft") or Input:key_pressed("AltRight")
                ...
                if snap then
                    delta = delta * 2
                    delta = delta+vec2(0.5,0.5)
                    delta = vec2(math.floor(delta.x),math.floor(delta.y))
                    delta = delta / 2
                end
                local position = input.start_position + delta
                if dupe then
                     -- unimplemented, will duplicate object and move duplicate in opposite direction
                end
        ...
        runtime_eval{
            ...
            code = [[
                ...
                if input.was_static == false then
                    obj:set_linear_velocity(input.point - input.prev_point)
                end
            ]]
        }
    end
